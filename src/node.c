    ...
    enum CAS_Player player)
{
    ...
    root = (struct CAS_Node*)GetMemory(
        mem,
        sizeof(struct CAS_Node));
    ...
    size_t maxActions)
{
    ...
    list = (struct CAS_NodeList*)GetMemory(
        mem,
        sizeof(struct CAS_NodeList));

    if (list == NULL)
        return NULL;

    list->nodes = (struct CAS_Node*)GetMemory(
        mem,
        maxActions*sizeof(struct CAS_Node));
